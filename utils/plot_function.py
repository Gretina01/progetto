...
def generate_free_parks_in_hours_for_day(day: int, parking: Parking):
        giorni = [data.day for data in [datetime.datetime.strptime(coppia[0], "%Y-%m-%d %H:%M:%S") for coppia in parking.get_detections_list()]]
        # Selezioniamo le coppie del mese di dicembre
        coppie_giorno = [coppia for coppia, giorno in zip(parking.get_detections_list(), giorni) if giorno == day]
        ore = [data.hour for data in [datetime.datetime.strptime(coppia[0], "%Y-%m-%d %H:%M:%S") for coppia in coppie_giorno]]

        occupati_per_fasce_orarie = [[] for _ in range(24)]

        for coppia, ora in zip(coppie_giorno, ore):
                occupati_per_fasce_orarie[ora].append(coppia)
...
def generate_figure_and_plot_for_month(avg_free_spaces_by_time_slot: list, month: int, output_name_file: str, format_image: str):
        try:
...
def generate_figure_and_plot_for_day(avg_free_spaces_by_time_slot: list, day: int, output_name_file: str, format_image: str):
        try:
                validate_free_spaces_by_time_slot(avg_free_spaces_by_time_slot)
        except ValueError as value:
                print(value)
                ...
                for i, valore in enumerate(avg_free_spaces_by_time_slot):
                        plt.text(i, valore, f"{valore:.2f}%", ha='center', va='bottom')
                        plt.title(f"Parcheggi liberi per fascia oraria nel giorno {Day(day).name}")
                        plt.ylabel("Numero di parcheggi liberi")
                        plt.xlabel("Orario")
